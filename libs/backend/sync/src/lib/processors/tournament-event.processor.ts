...
import { GameSyncJobData, StructureSyncJobData, JOB_TYPES, TOURNAMENT_EVENT_QUEUE } from '../queues/sync.queue';
import {
...
export class TournamentEventProcessor extends WorkerHost {
  ...
  async process(job: Job<StructureSyncJobData | GameSyncJobData, void, string>, token: string): Promise<void> {
    if (job.name.includes('tournament-sync-structure')) {
      ...
      await this.tournamentSubEventSyncService.processSubEventSync(job.data, job.id?.toString() || '', job.queueQualifiedName, async (progress: number) => {
        await job.updateProgress(progress);
      });
    } else if (job.name.includes('tournament-draw-')) {
      await this.tournamentDrawSyncService.processDrawSync(job.data as TournamentDrawSyncData, job.id?.toString() || '', job.queueQualifiedName, async (progress: number) => {
        await job.updateProgress(progress);
      });
