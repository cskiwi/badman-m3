...
export function langulageInitializer(translate: TranslateService, cookieService: SsrCookieService) {
  return async () => {
    ...
    try {
      ...
      const savedLang = (cookieService.get('translation.language') as AvaliableLanguages) || AvaliableLanguages.nl_BE;
...
export async function setLanguage(translateFormat: string, translateService: TranslateService, cookieService: SsrCookieService) {
  // Set cookie
