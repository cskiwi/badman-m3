...
export class KoChart<T> implements AfterViewInit, OnDestroy {
  ...
  /**
   * Whether to enable pan and zoom functionality for the chart.
   */
  readonly panZoomEnabled = input<boolean>(true);
  ...
  onToggleCollapse({
    ...
    if (highlightNode && this.panZoomEnabled()) {
      setTimeout(() => {
        const nodeElement = this.#elementRef?.nativeElement.querySelector(
          `#${
            wasCollapsed
  ...
  protected zoom({
    ...
    if (!this.panZoomEnabled() || !this.panZoomInstance) return;
